...
export class EventService {
  async getEvents(query?: string, max?: number) {
    ...
    if (response.status !== 200) {
      if (response.status === 401)
        return { error: 'Não autorizado para consultar os eventos' };
  ...
  async getEvent(id: string): Promise<{ error: string } | Event> {
    ...
    if (response.status !== 200) {
      if (response.status === 401)
        return { error: 'Não autorizado para consultar o evento' };
  ...
  async getTicketSales(id: string): Promise<{ error: string } | TicketSale[]> {
    ...
    if (response.status !== 200) {
      if (response.status === 401)
        return {
          error: 'Não autorizado para consultar os ingressos do evento',
        };
      return {
        error: 'Ocorreu um erro interno ao consultar os ingressos a venda',
      };
    }
