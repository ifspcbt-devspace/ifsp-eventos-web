...
export class EnrollmentService {
  ...
  async listCustomerEnrollments() {
    ...
    if (response.status !== 200) {
      if (response.status === 401)
        return { error: 'Não autorizado para listar inscrições' };
  ...
  async enroll(eventId: string, ticketSaleId: string) {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/order/pay`,
      {
        method: 'POST',
        body: JSON.stringify({
          event_id: eventId,
          items: [{ ticket_sale_id: ticketSaleId }],
        }),
        headers: {
    ...
    if (response.status !== 200) {
      ...
      if (response.status === 401)
        return { error: 'Não autorizado para inscrever-se' };
