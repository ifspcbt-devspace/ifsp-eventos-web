...
import {generateRandomUsername, notifyError, notifySuccess, notifyWarn} from "@/utils";
...
function Register() {
  ...
  const handleRegister = async () => {
    ...
    if (resp && resp.error) {
      ...
      if (respLogin && respLogin.error) {
        ...
        notifySuccess({description: "Conta criada com sucesso, redirecionando..."});
  ...
  const handleOpenModal = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (email === "" || password === "" || confirmPassword === "" || confirmEmail === "" || name === "" || birthDate === "" || document === "" || phone === "") {
      ...
      <div className="p-7 md:p-10 bg-white rounded-xl shadow-sm max-w-[380px]">
        <form className="block mt-0" autoComplete="off" onSubmit={handleOpenModal}>
          ...
          <Input maxLength={128} placeholder="Nome completo" name="name" title="Nome completo" onValueChange={setName}
                 ...
                 classNames={{inputWrapper: "rounded-[9px]", base: "mb-2"}} type="text" autoComplete="off"
                 isRequired={true}/>
          ...
          <Input maxLength={128} placeholder="E-mail" name="email" title="E-mail"
                 ...
                 classNames={{inputWrapper: "rounded-[9px]", base: "mb-2"}} type="email" autoComplete="off"
                 isRequired={true}/>

          <Input maxLength={128} placeholder="Confirmar E-mail" name="confirm_email" title="Confirmar E-mail"
                 isInvalid={isEmailInvalid} errorMessage="Os e-mails não correspoondem" onValueChange={setConfirmEmail}
                 classNames={{inputWrapper: "rounded-[9px]", base: "mb-2"}} type="email" autoComplete="off"
                 onPaste={(e) => {
                   e.preventDefault();
                   notifyWarn({description: "Por favor, digite seu e-mail manualmente para evitar erros."});
                 }
                 }
                 isRequired={true}
          />
          ...
          <Input type={"date"} placeholder="Nascimento" autoComplete="off" name="date" title="Data de nascimento"
                 ...
                 isRequired={true} classNames={{inputWrapper: "rounded-[9px]", base: "mb-2"}}/>
